...
const api = axios.create({
  baseURL: 'http://172.23.123.249:7116',  // Same IP as backend!
...
export default function ChatPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-black overflow-x-hidden">
      {/* HEADER */}
      <header className="fixed top-0 w-full bg-white/5 backdrop-blur-2xl z-10 flex items-center justify-between px-4 py-3 shadow-sm">
        <button className="p-2 rounded-md hover:bg-gray-100">
          <Menu size={22} />
        </button>
        <h1 className="text-lg font-semibold tracking-tight">BrainSeek</h1>
        <div className="w-8" />
      </header>
